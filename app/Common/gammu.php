...
class gammu
{
    ...
    public $gammu = "/usr/local/bin/gammu";
    ...
    function __construct($gammu_bin_location = '', $gammu_config_file = '', $gammu_config_section = '')
    {
        ...
        if (!file_exists($this->gammu)) {
            ...
            $this->gammu = $gammu_config_section != '' ? $this->gammu.
                " -s ".(int) $gammu_config_section."" : $this->gammu;
        }
    }
    ...
    function Identify(&$response)
    {
        ...
        if (preg_match("#Error opening device|No configuration file found|Gammu is not installed#si",
            $this->unbreak($r), $s)) {
            $response = $r;
